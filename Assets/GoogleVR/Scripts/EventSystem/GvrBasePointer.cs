...
public abstract class GvrBasePointer : MonoBehaviour
{
        ...
        get
        {
            ...
            switch (OVRInput.GetActiveController())
            {
                case OVRInput.Controller.Touchpad:
                    ...
                    break;
                default:
                    isTriggerDown = OVRInput.GetDown(OVRInput.Button.One) || OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger);
                    break;
            }
            ...
            {
                isTriggerDown = OVRInput.GetDown(OVRInput.Button.One) || OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger);
            }
            else
                isTriggerDown = isTriggerDown || Input.GetMouseButtonDown(0);
#elif QUEST
            isTriggerDown = OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger);
#else
            isTriggerDown = OVRInput.GetDown(OVRInput.Button.One) || OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger);
#endif
        ...
        get
        {
            ...
            switch (OVRInput.GetActiveController())
            {
                case OVRInput.Controller.Touchpad:
                    ...
                    break;
                default:
                    isTriggering = OVRInput.Get(OVRInput.Button.One) || OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger);
                    break;
            }
            ...
            if (OVRPlugin.GetSystemHeadsetType().ToString().Contains("Rift"))
            {
                isTriggering = OVRInput.Get(OVRInput.Button.One) || OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.Get(OVRInput.Button.SecondaryIndexTrigger);
            }
            else
                isTriggering = isTriggering || Input.GetMouseButton(0);
#elif QUEST
            isTriggering = OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.Get(OVRInput.Button.SecondaryIndexTrigger);
#else
            isTriggering = OVRInput.Get(OVRInput.Button.One) || OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.Get(OVRInput.Button.SecondaryIndexTrigger);
#endif
            return isTriggering;
        ...
        get
        {
            ...
            switch (OVRInput.GetActiveController())
            {
                case OVRInput.Controller.Touchpad:
                    ...
                    break;
                default:
                    isTriggerUp = OVRInput.GetUp(OVRInput.Button.One) || OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger);
                    break;
            }
            ...
            if (OVRPlugin.GetSystemHeadsetType().ToString().Contains("Rift"))
            {
                isTriggerUp = OVRInput.Get(OVRInput.Button.One) || OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetUp(OVRInput.Button.SecondaryIndexTrigger);
            }
            else
                isTriggerUp = isTriggerUp || Input.GetMouseButtonUp(0);

#elif QUEST
            isTriggerUp = OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetUp(OVRInput.Button.SecondaryIndexTrigger);
#else
            isTriggerUp = OVRInput.GetUp(OVRInput.Button.One) || OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger);
#endif
    ...
    public virtual Vector2 TouchPos
    {
        ...
        {
#if UNITY_EDITOR || UNITY_STANDALONE
            return Input.mousePosition / 20f;
#else
            float multiplier = OVRInput.GetActiveController() == OVRInput.Controller.Touchpad ? 3.0f : 0.5f;
            Vector2 oculusTouchPos = Vector2.zero;
            if (OVRInput.GetActiveController() == OVRInput.Controller.Touchpad)
            {
                oculusTouchPos.x = (Input.mousePosition.x - 900f) / (1700f - 900f);
                oculusTouchPos.y = (Input.mousePosition.y - 400f) / (1100f - 400f);
                oculusTouchPos = oculusTouchPos*multiplier;
            }
            else
            {
                oculusTouchPos = OVRInput.Get(OVRInput.Axis2D
                        .PrimaryTouchpad); //This is in the range -1 to -1 where -1, -1 is bottom left.
                oculusTouchPos.x = oculusTouchPos.x * -1f;
                oculusTouchPos.y = oculusTouchPos.y * -1f;
                oculusTouchPos = (oculusTouchPos * multiplier) + new Vector2(0.5f, 0.5f);
                oculusTouchPos = (oculusTouchPos * multiplier) + new Vector2(0.5f, 0.5f);
                oculusTouchPos.x = Mathf.Clamp01(oculusTouchPos.x);
                oculusTouchPos.y = Mathf.Clamp01(oculusTouchPos.y);
            }
            return oculusTouchPos;
#endif
        }
    }
