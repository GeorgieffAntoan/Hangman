...
public class HMGameController:MonoBehaviour{
    ...
    public void Start() {
        ...
        //GetWord();    	
        StartCoroutine(WordText());				// Delayed part of the start so that the keyboard script has finished adding letters to allKeyboardLetters variable
    }
    ...
    private IEnumerator LoadWord()
    {
        ...
        photonView.RPC("GetWord", PhotonTargets.MasterClient);
    }
    ...
    private IEnumerator WordText()
    {
        ...
        photonView1.RPC("LateStart", PhotonTargets.MasterClient);
    }
    ...
    private IEnumerator TopicFinder()
    {
        ...
        photonView2.RPC("RandomTopic", PhotonTargets.MasterClient);
    }
    [PunRPC]
    ...
    public bool CheckLetter(string l) {					// Check if a letter is in the word
    	...
    	if(foundLetter){										// Check if a letter has been found
    		...
    		if(lettersFound >= currentWordLength){              // Check if the complete word has been solved
                ...
                photonView3.RPC("SolvedWord", PhotonTargets.MasterClient);                              // The word has been solved
            }
    	}else{
            photonView4 = GetComponent<PhotonView>();
            photonView4.RPC("WrongLetter", PhotonTargets.MasterClient);
